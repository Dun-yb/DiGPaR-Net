...
parser.add_argument('--strat_lr', type=float, default=0.0004, help='start, learning rate')
parser.add_argument('--end_lr', type=float, default=0.000001, help='end learning rate')
parser.add_argument('--no_lr_sche', action='store_true', help='no lr cos schedule')
parser.add_argument('--use_warm_up', type=bool, default=False, help='using warm up in learning rate')

parser.add_argument('--w_loss_L1', type=float, default=1., help='weight of loss L1')
parser.add_argument('--w_loss_CR', type=float, default=0.1, help='weight of loss CR')
...
if not os.path.exists(model_dir):
    os.mkdir(model_dir)
    opt.saved_model_dir = os.path.join(model_dir, 'save_model')
    opt.saved_data_dir = os.path.join(model_dir, 'save_data')
